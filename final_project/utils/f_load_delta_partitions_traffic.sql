--drop function std9_121.f_load_delta_partitions_traffic(text, text, timestamp, timestamp);
create or replace function std9_121.f_load_delta_partitions_traffic(p_table_from text, p_table_to text, p_start_date timestamp, p_end_date timestamp)
    returns int8
    language plpgsql
    security definer
    volatile
...
$$
DECLARE
	v_location 		text := 'std9_121.f_load_delta_partitions_traffic';
    v_table_from    text;
    v_table_to      text;
    v_start_date    date;
    v_end_date      date;
...
BEGIN
    ...
    LOOP
        ...
        PERFORM std9_121.f_write_log(p_log_type    := 'INFO',
                                     ...
                                     p_location    := v_location);
        -- Insert the data
        v_where = 'TO_DATE("date", ''DD.MM.YYYY'') >=''' || v_start_date || '''::timestamp and TO_DATE("date", ''DD.MM.YYYY'') <''' || v_iterDate || '''::timestamp';
        v_sql := 'INSERT INTO '||v_prt_table||' SELECT plant, TO_DATE("date", ''DD.MM.YYYY''), "time", frame_id, quantity FROM '||v_table_from||' WHERE '||v_where;
        EXECUTE v_sql;
